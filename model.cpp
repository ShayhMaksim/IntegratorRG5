...
void TDormandPrince::Run(TModel* Model)
{
	...
	while ( t < t1 )
    {
        ...
        long double *max=new long double[X.GetSize()];
        double e1=0;
        for(int i=0;i<X.GetSize();i++)
        {
            long double *max1=new long double[4]{(0.00001) , abs(X1[i]), abs(X[i]) , 0.25*u/Eps};
        ...
        if (5>pow((e/Eps), 0.1)/0.5) { min=pow(e/Eps, 0.1)/0.5; } else min=5;
